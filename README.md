...
- **Language** задаёт язык мода: Java, Kotlin или Groovy. Для Kotlin и Groovy
  их рантайм добавляется в JAR. Java отдельный рантайм не нужен.
...
Плагин показывает иконки возле наследников `SacredMod` и возле
...
который наследует `SacredMod` напрямую или через собственный абстрактный
базовый класс мода. Подсказка сообщает, совпадает ли класс с
...
распознаются классы верхнего уровня в пакете событий, кроме `EventMutation`,
`Decides`, `Fold` и `Delivery`. В подсказке указаны событие и нестандартный
приоритет обработчика, а у обработчика с `MONITOR` иконка серая. Нажатие
открывает класс события. Тип результата иконка не проверяет: `void` наблюдает,
а `Mutation` события решает, и это проверяет линтер при сборке.
